...
class CoordinatorAgentPrimitive(InstrumentedPrimitive[dict[str, Any], dict[str, Any]]):
    ...
    async def _execute_impl(self, input_data: dict[str, Any], context: WorkflowContext) -> dict[str, Any]:
        ...
        subtasks = [
            {"agent": "data_analyst", "task": f"Analyze data patterns in: {task}", "priority": "high"},
            {"agent": "researcher", "task": f"Gather background info on: {task}", "priority": "medium"},
            {"agent": "fact_checker", "task": f"Verify key claims for: {task}", "priority": "low"},
            {"agent": "summarizer", "task": f"Summarize findings for: {task}", "priority": "low"},
        ]
...
class DataAnalystAgentPrimitive(InstrumentedPrimitive[dict[str, Any], dict[str, Any]]):
    ...
    async def _execute_impl(self, input_data: dict[str, Any], context: WorkflowContext) -> dict[str, Any]:
        ...
        task = input_data.get("task", "")
        return {"agent": "data_analyst", "status": "success", "output": f"insights for [{task}]"}
...
class ResearcherAgentPrimitive(InstrumentedPrimitive[dict[str, Any], dict[str, Any]]):
    ...
    async def _execute_impl(self, input_data: dict[str, Any], context: WorkflowContext) -> dict[str, Any]:
        ...
        task = input_data.get("task", "")
        return {"agent": "researcher", "status": "success", "output": f"references for [{task}]"}
...
class FactCheckerAgentPrimitive(InstrumentedPrimitive[dict[str, Any], dict[str, Any]]):
    ...
    async def _execute_impl(self, input_data: dict[str, Any], context: WorkflowContext) -> dict[str, Any]:
        ...
        task = input_data.get("task", "")
        verified = True
        return {"agent": "fact_checker", "status": "success", "output": f"verified={verified} for [{task}]"}
...
class SummarizerAgentPrimitive(InstrumentedPrimitive[dict[str, Any], dict[str, Any]]):
    ...
    async def _execute_impl(self, input_data: dict[str, Any], context: WorkflowContext) -> dict[str, Any]:
        ...
        task = input_data.get("task", "")
        return {"agent": "summarizer", "status": "success", "output": f"summary for [{task}]"}
...
class AggregatorAgentPrimitive(InstrumentedPrimitive[list[dict[str, Any]], dict[str, Any]]):
    ...
    async def _execute_impl(self, input_data: list[dict[str, Any]], context: WorkflowContext) -> dict[str, Any]:
...
async def demo_multi_agent() -> None:
    ...
    coord_out = await coordinator._execute_impl({"task": "Analyze quarterly metrics", "task_id": "task-42"}, context)
