...
def make_resilient_llm(
    ...
    *,
    ...
    Composes a :class:`LiteLLMPrimitive` with TTA.dev's
    ...
    Args:
        provider: Provider prefix bound to the :class:`LiteLLMPrimitive`.  When
            ``None``, ``request.model`` must be a full litellm model string.
        litellm_fallbacks: Ordered list of litellm model strings to attempt when
            the primary call fails.
        api_key: API key override forwarded to :class:`LiteLLMPrimitive`.
        api_base: Base URL override forwarded to :class:`LiteLLMPrimitive`.
