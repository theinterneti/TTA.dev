...
class TestAnalyzeCommand:
    ...
    def test_analyze_quiet_flag(self, runner: CliRunner, sample_code_file: Path) -> None:
    ...
    def test_analyze_lines_flag(self, runner: CliRunner, complex_code_file: Path) -> None:
        """Verify --lines shows line numbers for issues."""
        result = runner.invoke(app, ["analyze", str(complex_code_file), "--lines", "--quiet"])
        assert result.exit_code == 0
    ...
    def test_analyze_suggest_diff_flag(self, runner: CliRunner, complex_code_file: Path) -> None:
    ...
    def test_analyze_json_with_lines(self, runner: CliRunner, complex_code_file: Path) -> None:
...
'''
    ...
    def test_analyze_apply_unknown_primitive(
        ...
        assert "Unknown primitive" in result.stdout or "unknown" in result.stdout.lower()
...
class TestTransformCommand:
    ...
    def test_transform_retry_primitive(self, runner: CliRunner, complex_code_file: Path) -> None:
    ...
    def test_transform_cache_primitive(self, runner: CliRunner, complex_code_file: Path) -> None:
    ...
    def test_transform_specific_function(self, runner: CliRunner, complex_code_file: Path) -> None:
    ...
    def test_transform_diff_output(self, runner: CliRunner, complex_code_file: Path) -> None:
    ...
    def test_transform_unknown_primitive(self, runner: CliRunner, sample_code_file: Path) -> None:
        """Verify transform handles unknown primitive."""
        result = runner.invoke(app, ["transform", str(sample_code_file), "UnknownPrimitive"])
        assert result.exit_code == 1
        assert "Unknown primitive" in result.stdout or "unknown" in result.stdout.lower()

    def test_transform_file_not_found(self, runner: CliRunner) -> None:
        """Verify transform handles non-existent file."""
        result = runner.invoke(app, ["transform", "nonexistent_file.py", "RetryPrimitive"])
        assert result.exit_code != 0
