...
async def handle_api_workflows(request):
    ...
    if workflows_dir.exists():
        for wf_file in workflows_dir.glob("*.yml"):
            try:
                workflows.append(
                    {"name": wf_file.stem, "file": str(wf_file.name), "type": "github_action"}
                )
            except Exception:
                pass
        ...
        for wf_file in workflows_dir.glob("*.md"):
            try:
                workflows.append(
                    {"name": wf_file.stem, "file": str(wf_file.name), "type": "agentic_workflow"}
                )
            except Exception:
                pass
            ...
            if TRACES_DIR.exists():
                ...
                for trace_file in new_traces:
                    try:
                        with open(trace_file) as f:
                            trace = json.load(f)
                            await broadcast_to_websockets({"type": "new_trace", "trace": trace})
                    except Exception as e:
...
async def handle_api_cgc_graph(request):
    ...
    try:
        ...
        if "error" in graph_data:
            response = web.json_response(
                {"success": False, "error": graph_data["error"]}, status=500
            )
        else:
