...
async def test_retry_success_after_failures():
    ...
    retry_primitive = RetryPrimitive(mock_primitive, strategy=RetryStrategy(max_retries=3, backoff_base=0.01))
    context = WorkflowContext()
...
async def test_retry_exhausted():
    ...
    mock_primitive = MockPrimitive(name="mock_primitive", raise_error=ValueError("persistent failure"))
    retry_primitive = RetryPrimitive(mock_primitive, strategy=RetryStrategy(max_retries=2, backoff_base=0.01))
    context = WorkflowContext()
...
async def test_retry_context_is_passed_correctly():
    ...
    def side_effect_handler(*args, **kwargs):
        ...
        return effect
    mock_primitive.side_effect = side_effect_handler
    retry_primitive = RetryPrimitive(mock_primitive, strategy=RetryStrategy(max_retries=1, backoff_base=0.01))
