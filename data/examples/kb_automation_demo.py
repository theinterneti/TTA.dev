...
sys.path.insert(0, str(Path(__file__).parent.parent / "platform" / "kb-automation" / "src"))
...
async def run_demo():
    ...
    print(f"   Code Path: {code_path} ({'✅ exists' if code_exists else '❌ not found'})")
    ...
    try:
        if code_exists:
            ...
            with tempfile.TemporaryDirectory() as temp_kb:
                ...
                print(f"   Docstrings extracted: {result.get('docstrings_extracted', 0)}")
