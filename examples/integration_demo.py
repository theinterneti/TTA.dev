...
sys.path.insert(0, os.path.join(os.path.dirname(__file__), "..", "platform", "observability", "src"))
sys.path.insert(0, os.path.join(os.path.dirname(__file__), "..", "platform", "agent-context", "src"))
...
async def demo_core_primitives():
    ...
    start = time.time()
    result1 = await cached_workflow.execute({"query": "test"}, context)
    time1 = time.time() - start

    start = time.time()
    result2 = await cached_workflow.execute({"query": "test"}, context)
...
async def demo_agent_context():
    ...
    retrieve_decision = AgentMemoryPrimitive(
        operation="retrieve", memory_key="architecture_choice"
    )
...
async def demo_full_integration():
    ...
    print(f"   - Retry protection: Active")
    print(f"   - Cache: Active (60s TTL)")
    print(f"   - Timeout: 5s max")
    print(f"   - Routing: Smart (environment-aware)")
    print(f"   - Multi-agent: 2 validators")
