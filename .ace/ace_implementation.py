...
class ACEKnowledgeCapture:
    ...
    def _extract_code_patterns(self, tree: ast.AST, file_path: Path, content: str) -> List[DevelopmentPattern]:
    ...
    def generate_session_report(self, session_data: Dict[str, Any]):
        ...
        with open(report_file, 'w') as f:
            ...
            for pattern in session_data.get('captured_patterns', []):
                f.write(f"- **{pattern.get('name', 'Unknown')}**: {pattern.get('description', '')}\n")

            f.write("\n## Integration Learnings\n\n")
            for learning in session_data.get('integration_learnings', []):
                f.write(f"- **{learning.get('integration_type', 'Unknown')}**: {learning.get('challenge', '')}\n")
