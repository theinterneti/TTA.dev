...
def test_model(
    model_name: str, quantization: str = "4bit", temperature: float = 0.7, max_new_tokens: int = 200
) -> dict[str, Any]:
    """
    ...
    try:
        ...
        for prompt_type, prompt in TEST_PROMPTS.items():
            ...
            inputs = tokenizer(
                full_prompt, return_tensors="pt", padding=True, truncation=True, max_length=512
            )
            ...
            logger.info(
                f"Attention mask sum: {inputs['attention_mask'].sum().item()} (should match non-padding tokens)"
            )
        ...
        return {
            "model": model_name,
            "config": {"quantization": quantization, "temperature": temperature},
            "timestamp": datetime.now().isoformat(),
            "error": str(e),
        }
    ...
    for model in models:
        for quantization in quantizations:
            for temperature in temperatures:
                logger.info(
                    f"Testing {model} with quantization={quantization}, temperature={temperature}"
                )

                # Run test
                result = test_model(model, quantization=quantization, temperature=temperature)
...
def print_analysis(analysis: dict[str, Any]):
    ...
    for model, configs in analysis["best_configurations"].items():
        ...
        for metric, config in configs.items():
            if config:
                print(
                    f"  Best for {metric}: quantization={config['quantization']}, temperature={config['temperature']}"
                )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Test language models with different configurations."
    )
    ...
    parser.add_argument(
        "--temperatures", nargs="+", type=float, help="Temperature settings to test"
    )
