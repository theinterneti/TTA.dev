...
def update_pyproject_paths(repo_root: Path, dry_run: bool = True) -> None:
    ...
    replacements = [
        (
            'packages = ["platform/primitives/src"]',
            'packages = ["packages/tta-primitives/src"]',
        ),
        (
        ...
        (
            ...
            'packages = ["packages/tta-secrets/src"]',
        ),
    ...
    for old, new in replacements:
        if old in updated:
            print(
                f"{'[DRY-RUN] ' if dry_run else ''}Update pyproject.toml: {old} → {new}"
            )
            if not dry_run:
...
def main():
    parser = argparse.ArgumentParser(
        description="Consolidate TTA.dev repository structure"
    )
    parser.add_argument(
