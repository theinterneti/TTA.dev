...
def visualize_results(results, output_dir=None):
    ...
    for model_name, results_list in model_results.items():
        ...
        load_times = [result["model_load_time"] for result in results_list if "model_load_time" in result]
    ...
    for model_name, results_list in model_results.items():
        ...
        memory_usages = [result["memory"]["model_size_mb"] for result in results_list if "memory" in result and "model_size_mb" in result["memory"]]
    ...
    for model_name, results_list in model_results.items():
        ...
        for prompt_type in prompt_types:
            ...
            model_prompt_tps[short_name][prompt_type] = sum(tps_values) / len(tps_values) if tps_values else 0
    ...
    plt.ylabel('Tokens per Second')
    plt.xticks(index + bar_width * (len(model_prompt_tps) - 1) / 2, prompt_types, rotation=45, ha='right')
    plt.legend()
    plt.tight_layout()
