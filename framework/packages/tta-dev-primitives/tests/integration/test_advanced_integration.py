...
async def test_full_recovery_chain():
    ...
    # Build the workflow
    retry_primitive = RetryPrimitive(flaky_slow_primitive, strategy=RetryStrategy(max_retries=1, backoff_base=0.01))
    timeout_primitive = TimeoutPrimitive(retry_primitive, timeout_seconds=0.1, fallback=fast_fallback)
