...
async def test_tool_use(model_name: str, test_case: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        if "expected_tools" in test_case:
            ...
            result["tools_mentioned_rate"] = len(tools_mentioned) / len(expected_tools) if expected_tools else 0
                ...
                try:
                    ...
                    for key, value in expected_params.items():
                        if key not in params:
                            ...
                            isinstance(value, (int, float)) and isinstance(params[key], (int, float))
                        ):
                            params_correct = False
        ...
        if "expected_tool_calls" in test_case:
            ...
            tool_matches = re.finditer(r"<tool>(.*?)</tool>\s*<parameters>\s*(\{.*?\})\s*</parameters>", response, re.DOTALL)
            ...
            result["tool_calls_match_rate"] = matched_calls / len(expected_calls) if expected_calls else 0
...
async def run_tests(models: List[str] = None) -> Dict[str, Any]:
    ...
    for model in models:
        ...
        for test_case in TOOL_USE_TESTS:
            ...
            if result["success"]:
                if "tools_mentioned_rate" in result:
                    logger.info(f"    Tools mentioned: {result['tools_mentioned_count']}/{result['tools_expected_count']} ({result['tools_mentioned_rate'] * 100:.1f}%)")
                elif "overall_correct" in result:
                    correct_str = "Correct" if result["overall_correct"] else "Incorrect"
                    logger.info(f"    Tool call: {correct_str}, Duration: {result['duration']:.2f}s")
                elif "tool_calls_match_rate" in result:
                    logger.info(f"    Tool calls matched: {result['matched_tool_calls']}/{result['expected_tool_calls_count']} ({result['tool_calls_match_rate'] * 100:.1f}%)")
                else:
        ...
        avg_tool_mention_rate = sum(r["tools_mentioned_rate"] for r in tool_mention_results) / len(tool_mention_results) if tool_mention_results else 0

        # Tool call correctness
        tool_call_results = [r for r in model_results if "overall_correct" in r]
        tool_call_correct_rate = sum(1 for r in tool_call_results if r.get("overall_correct", False)) / len(tool_call_results) if tool_call_results else 0

        # Multiple tool calls
        multi_tool_results = [r for r in model_results if "tool_calls_match_rate" in r]
        avg_tool_calls_match_rate = sum(r["tool_calls_match_rate"] for r in multi_tool_results) / len(multi_tool_results) if multi_tool_results else 0

        # Average duration
        avg_duration = sum(r["duration"] for r in model_results if r["success"]) / sum(1 for r in model_results if r["success"]) if sum(1 for r in model_results if r["success"]) > 0 else 0
    ...
    for model in models:
        ...
        for complexity in ["low", "medium", "high"]:
            ...
            if complexity_results:
                ...
                complexity_tool_mention_results = [r for r in complexity_results if "tools_mentioned_rate" in r]
                complexity_tool_mention_rate = sum(r["tools_mentioned_rate"] for r in complexity_tool_mention_results) / len(complexity_tool_mention_results) if complexity_tool_mention_results else 0

                complexity_tool_call_results = [r for r in complexity_results if "overall_correct" in r]
                complexity_tool_call_rate = sum(1 for r in complexity_tool_call_results if r.get("overall_correct", False)) / len(complexity_tool_call_results) if complexity_tool_call_results else 0
    ...
    for complexity in ["low", "medium", "high"]:
        ...
        for model in models:
            ...
            if model_complexity_results:
                ...
                model_success_rate = sum(1 for r in model_complexity_results if r["success"]) / len(model_complexity_results)

                # Tool metrics for this model at this complexity
                model_tool_mention_results = [r for r in model_complexity_results if "tools_mentioned_rate" in r]
                model_tool_mention_rate = sum(r["tools_mentioned_rate"] for r in model_tool_mention_results) / len(model_tool_mention_results) if model_tool_mention_results else 0

                model_tool_call_results = [r for r in model_complexity_results if "overall_correct" in r]
                model_tool_call_rate = sum(1 for r in model_tool_call_results if r.get("overall_correct", False)) / len(model_tool_call_results) if model_tool_call_results else 0
...
def print_analysis(analysis: Dict[str, Any]):
    ...
    for model, perf in analysis["model_performance"].items():
        ...
        for complexity, comp_perf in perf["complexity_performance"].items():
            print(f"    {complexity.upper()}: Success: {comp_perf['success_rate'] * 100:.1f}%, Tool Mention: {comp_perf['tool_mention_rate'] * 100:.1f}%, Tool Call: {comp_perf['tool_call_correct_rate'] * 100:.1f}%")

    print("\n----- COMPLEXITY PERFORMANCE -----")
    for complexity, perf in analysis["complexity_performance"].items():
        print(f"\n{complexity.upper()}:")
        for model, model_perf in sorted(perf["model_performance"].items(), key=lambda x: (x[1]["tool_call_correct_rate"] + x[1]["tool_mention_rate"]) / 2, reverse=True):
            print(f"  {model}: Success: {model_perf['success_rate'] * 100:.1f}%, Tool Mention: {model_perf['tool_mention_rate'] * 100:.1f}%, Tool Call: {model_perf['tool_call_correct_rate'] * 100:.1f}%")
